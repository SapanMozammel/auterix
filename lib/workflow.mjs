...
// Must stay a plain literal, not a package.json lookup: this file is copied
// byte-for-byte into consumer projects as .ai/tools/workflow.mjs, where no
// accompanying package.json exists at a predictable relative path.
// test/workflow.test.mjs asserts this matches package.json's version.
...
// Required since 1.0.0. Never add here directly — a bundle's required-files
// check (validateBundle) applies this unconditionally to every version,
// including the immutable historical 1.0.0 fixture, so a new unconditional
// entry breaks legacy-bundle installs. Add new project-owned files to
// PROJECT_1_2_1 (or a future version-gated list) instead, mirroring how
// REQUIRED_12 gates new managed files behind strictVersion.
const PROJECT_CORE = [
  'AGENTS.md',
  '.ai/manifest.json',
  '.ai/project.json',
...
const PROJECT_1_2_1 = ['.formatter/FORMATTER_CONFIG.md', '.formatter/sync.cjs', '.formatter/README.md'];
const PROJECT = new Set([...PROJECT_CORE, ...PROJECT_1_2_1]);
function requiredProject(version) {
  return [...PROJECT_CORE, ...(strictVersion(version) ? PROJECT_1_2_1 : [])];
}
...
function allowed(file, ownership) {
  ...
  return (
    ...
    (BRIDGES.has(file) ||
      ...
      /^\.ai\/(core|templates|tools|roles)\/[a-zA-Z0-9_./-]+\.(md|json|mjs)$/.test(file))
  );
}
...
export function validateBundle(bundle) {
  ...
  if (
    ![...requiredProject(bundle.version), ...requiredManaged(bundle.version)].every((file) =>
      seen.has(file.toLowerCase()),
    )
  )
