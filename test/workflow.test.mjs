...
test('legacy consumers migrate through a reviewed update and retain their project state', (t) => {
  ...
  // A new current packet adopts the explicit task contract; old history remains unchanged.
  manifest.currentTask = '.ai/tasks/upgrade.md';
  write(
    root,
    manifest.currentTask,
    bundle.files.find((file) => file.path === '.ai/tasks/current.md').content,
  );
...
test(
  'a mid-write permission failure rolls back installation without touching the blocked directory',
  ...
  (t) => {
    ...
    try {
      assert.throws(() => applyPlan(root, bundle, plan), /rolled back/);
      assert(!fs.existsSync(path.join(root, '.ai/workflow.writer.json')));
      assert(!fs.existsSync(path.join(root, '.ai/core/start.md')));
      assert(!fs.existsSync(path.join(root, '.ai/workflow.lock.json')));
      assert.equal(fs.readdirSync(blockedDirectory).length, 0);
    } finally {
