...
try {
  ...
  for (let i = 0; i < args.length; i += 2) {
    const key = args[i];
    if (
      !['--root', '--out', '--plan', '--bundle'].includes(key) ||
      !args[i + 1] ||
      args[i + 1].startsWith('--') ||
      key in options
    )
  ...
  switch (command) {
    ...
    case 'inspect':
      ...
      break;
    case 'plan':
      result = makePlan(options['--root'], bundle());
      break;
    case 'update-plan':
    ...
    default:
      throw new Error(
        'Usage: node bin/workflow.mjs inspect|plan|update-plan|apply|check|eject-plan --root /absolute/project [--bundle /absolute/bundle.json] [--plan /absolute/plan.json] [--out /absolute/new.json]; or bundle --out /absolute/new.json',
      );
  }
